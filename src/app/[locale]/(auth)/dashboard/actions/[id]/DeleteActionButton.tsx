...
import { deleteAction } from '@/actions/supabase';
import { buttonVariants } from '@/components/ui/buttonVariants';

export function DeleteActionButton({ actionId, actionTitle }: { actionId: string; actionTitle: string }) {
  ...
  if (showConfirm) {
    return (
      <div className="flex flex-col gap-3 rounded-lg border border-destructive/50 bg-destructive/5 p-4">
        <p className="text-sm">
          Delete action: <strong>{actionTitle}</strong>?
        </p>
